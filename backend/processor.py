...
def process_pdf(file_path: str): #defining a function that converts a pdf to string and then make chunks, these chunks would be sent to vector DB
    ...
    chunks = text_splitter.split_documents(pages) #takes in the fulltext created using the join method above and then use split_text using the method text_splitter defined above
    return chunks